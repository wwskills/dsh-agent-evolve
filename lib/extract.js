 ...
 *
 * Wraps `streamLlm(llm, { model, messages, signal })` with the
 ...
 */
export async function extractRule({ llm, corrections, cfg = {}, signal, model } = {}) {
  if (!llm) return null;
 ...
 */
export function buildPersonaPrompt(memories, _cfg = {}) {
 ...
 */
export async function extractPersona({ llm, memories, cfg = {}, signal, model } = {}) {
  if (!llm) return null;
