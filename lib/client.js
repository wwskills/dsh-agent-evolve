window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ─── MemoriesTab ─────────────────────────────────────────────────────
		function MemoriesTab({ t, badges, setBadges }) {
			var _useState = react.useState, useState = _useState[0], setState = _useState[1];
			var _useEffect = react.useEffect;
		...
		// ─── PersonaTab ──────────────────────────────────────────────────────
		function PersonaTab({ t, badges, setBadges }) {
			var _useState = react.useState, useState = _useState[0], setState = _useState[1];
			var _useEffect = react.useEffect;
		...
		function AgentEvolveSettingsTab({ t }) {
			...
			_useEffect(function() {
				...
				fetch('/plugins/agent-evolve/api/stats')
					.then(function(r) { return r.json(); })
					.then(function(data) { setStats(data); setBadges({ lessons: data.corrections_pending || 0, rules: data.rules_proposed || 0, memories: data.memories_extracted || 0, persona: 0 }); })
					.catch(function() {});
			}, []);
