window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			statsNever: "从未",
			statsPaused: "已暂停",
			// P3.25: monthly stats line (Dashboard 月度统计)
			statsMonthlyLabel: "本月",
			statsMonthlyLessons: "教训",
			statsMonthlyExtractions: "抽取",
			statsMonthlyProposed: "提炼",
			statsMonthlyApproved: "批准",
			// P3.23: rule conflict warning
		...
		const en = {
			...
			statsPaused: "Paused",
			// P3.25: monthly stats line (Dashboard 月度统计)
			statsMonthlyLabel: "This month",
			statsMonthlyLessons: "lessons",
			statsMonthlyExtractions: "extractions",
			statsMonthlyProposed: "proposed",
			statsMonthlyApproved: "approved",
			// P3.23: rule conflict warning
		...
		function RulesTab({ t, badges, setBadges, onJumpToCorrection }) {
			...
			function handleApprove(rule) {
				...
				fetch('/plugins/agent-evolve/api/rules/' + rule.id + '/approve', { method: 'POST' })
					...
					.then(function(data) {
						// P3.23: surface conflict warnings from the server.
						// The server has already approved the rule by this
						// point — the warning is informational, not a
						// gate. We show a confirm so the user can decide
						// whether to keep it; "Cancel" is a UI-only state
						// (we don't undo the approval server-side because
						...
						if (data && data.warning && Array.isArray(data.warning.conflicts) && data.warning.conflicts.length > 0) {
							...
							if (typeof window !== "undefined" && window.confirm) {
								...
								console.warn('[agent-evolve] rule conflict:', body);
							}
						}
						loadRules();
						refreshBadges();
		...
		function BottomStatsBar({ stats, t, enabled }) {
			...
			return js("div", {
				...
				children: [
					...
					// P3.25: 本月统计 line (monthly dashboard).
					// Surfaces current-month counters from usage_stats so
					// the user can see this month's learning momentum at a
					// glance. The four numbers map to the four bumpUsage
					// call sites: insertCorrection / runMemoryExtraction /
					// runRuleExtraction / approveRule.
					js("span", { key: "l4", style: { display: "flex", gap: "16px", marginLeft: "auto", color: "var(--dsw-alias-label-tertiary)" }, children: [
						j("span", { key: "l4a", children: [t("statsMonthlyLabel"), "：", t("statsMonthlyLessons"), " ", j("b", { style: { color: "var(--dsw-alias-label-primary)" }, children: s.monthly_corrections_captured || 0 }), " │ ", t("statsMonthlyExtractions"), " ", j("b", { style: { color: "var(--dsw-alias-label-primary)" }, children: s.monthly_extractions || 0 }), " │ ", t("statsMonthlyProposed"), " ", j("b", { style: { color: "var(--dsw-alias-label-primary)" }, children: s.monthly_rules_proposed || 0 }), " │ ", t("statsMonthlyApproved"), " ", j("b", { style: { color: "var(--dsw-alias-label-primary)" }, children: s.monthly_rules_approved || 0 })] }),
					]}),
				],
			});
		}
